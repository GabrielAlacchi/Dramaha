...
const maybeTransition = (potSplit) => {
  if (potSplit.style.transform === "") {
    ...
    setTimeout(() => potSplit.style.display = "none", 1200);
  }
}

window.onload = () => {
  const potRootEl = document.getElementById('pot');

  const config = { attributes: true, childList: true, subtree: true };

  const observer = new MutationObserver(() => {
    Array.from(potRootEl.children).forEach((potEntry) => {
      if (potEntry.classList.contains("won")) {
        for (const potSplit of potEntry.children) {
  ...
  observer.observe(potRootEl, config);
};
